import subprocess
import requests
from dev.utils.constants import SONARR_SCHEMA_URL, SONARR_SCHEMA_PATH, CLIENT_OUTPUT_DIR, SONARR_CLIENT_CONFIG_PATH
import json
...
def generate_client():
    ...
    subprocess.run([
        ...
        "generate", 
        "--config", str(SONARR_CLIENT_CONFIG_PATH),
        "--output-path", str(CLIENT_OUTPUT_DIR),
        "--path", str(SONARR_SCHEMA_PATH),
        "--overwrite"
    ], check=True)
